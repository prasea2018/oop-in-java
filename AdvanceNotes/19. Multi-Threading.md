...
* **Problem:**
  ...
  * If a thread checks the mutex but is preempted before locking, another thread may also start using the resource → **race condition**.
...
# **📘What is Monitor: Synchronization Using `synchronized`**
...
* Producer writes → Consumer waits
* Producer finishes → Consumer reads → Producer waits
