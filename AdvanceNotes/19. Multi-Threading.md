...
* **Problem:**
  ...
  * If a thread checks the mutex but is preempted(time slice over) before locking, another thread may also start using the resource → **race condition**.
