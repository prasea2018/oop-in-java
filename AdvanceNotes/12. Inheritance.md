...
# **📘 Inheritance in Java — Generalization & Specialization Explained Clearly**
...
Both concepts form a **hierarchy** (parent → children), but they work in opposite directions.

---

# **📍 A. Generalization (Bottom-Up Approach)**

Generalization means **grouping several specific things into one common category**.

### **Examples**

* iPhone, Samsung, Vivo → grouped together as **Smartphones**
* Car, Bike, Ship → grouped together as **Vehicles**
* Triangle, Rectangle, Circle → grouped as **Shapes**
...
Specific → Common
iPhone + Samsung + Vivo → Smartphone
...
# **📍 B. Specialization (Top-Down Approach)**
...
| ------------------ | --------- | ----------------------------- | ----------------------------- |
| **Generalization** | Bottom-Up | iPhone + Samsung → Smartphone | Interfaces / Abstract classes |
| **Specialization** | Top-Down  | Car → Fortuner                | Inheritance (`extends`)       |
...
# **📘 Inheritance in Java — Cleaned & Structured Notes**
...
* `Circle` → has **radius**
* `Cylinder` → has **radius** (inherited from Circle) + **height**
...
* c2.area() ✔ (inherited)
* c2.perimeter() ✔ (inherited)
* c2.volume() ✔ (own method)
...
All inherited properties and methods *exist inside the child class*.

### **(2) Accessible**

Whether the child class can actually *use* them depends on access modifiers:

* `public` → available + accessible
* `protected` → available + accessible inside child
* `default` → sometimes not accessible
* `private` → available but **NOT accessible**

Example analogy:

* Your friend gives you a box.
* You can **hold** it → *available*
* You cannot **open** it → *not accessible*
...
* Achieved using **interfaces** or abstract classes
  Examples:
* iPhone, Samsung, MiPhone → Smartphone
* Car, Bike, Ship → Vehicle
...
# **📘 Calling Parameterized Constructors of the Parent Class in Java Inheritance**

In this lesson, we learn:

✔ How constructors behave in inheritance
✔ How to call the **parameterized constructor** of a parent class using **super(...)**
✔ Why `super(...)` must be the *first* statement
✔ Examples using simple classes, and later Rectangle → Cuboid
...
* Again, no `super(...)` is written → Java calls `super()` by default
* Then Child(int y) runs

---

# **3. Calling the Parent’s Parameterized Constructor**
...
✔ It MUST be the **first line** inside the child constructor
✔ You can pass any values you want
...
# **5. Rectangle → Cuboid Example (Practical Scenario)**
...
* parent: calls Rectangle() → length = 1, breadth = 1
* height = 10
...
1. `super(5, 3)` → sets length = 5, breadth = 3
2. height = 10

So volume = 5 × 3 × 10 = **150**
...
### ✔ To call parent **parameterized constructor**, write `super(parameters)` manually

### ✔ `super(...)` must be the *first* line in the constructor

### ✔ Construction always happens **top → down**
...
1. `this` → reference to the **current object**
2. `super` → reference to the **parent (super) class**
...
* So `r1.display()` → `this` refers to `r1`
* `r2.display()` → `this` refers to `r2`

✅ **Summary:** `this` always refers to the **current object**.

---

## **2️⃣ The `super` Keyword**
...
* `super.x` → 10 (from Rectangle)
* `x` → 20 (from Cuboid)

---

## **3️⃣ Summary of `this` vs `super`**
...
## **1️⃣ Example of Method Overriding**
...
## **2️⃣ Calling Overridden Methods**
...
✅ **Rule:** In Java, **method calls are resolved at runtime based on the actual object**, not the reference type.
...
* Reference type **does not matter** for which method is executed—**object type matters**.
...
### **5️⃣ Recap: Overriding vs Overloading**
...
1. Method overriding → subclass provides **its own version** of a superclass method.
2. Dynamic method dispatch → **method call depends on object type at runtime**, not reference type.
3. Overridden superclass methods are **shadowed** by subclass methods.
4. Overloading → compile-time, different signatures, not related to dynamic dispatch.
5. Real-life analogy → reference = generic interface, object = actual behavior.
...
## **2️⃣ Superclass Reference, Subclass Object**
...
* ❌ The reverse is **not allowed**: `SubClass subRef = new SuperClass();` is invalid.

**Why allowed?**

* Because the subclass **is a type of superclass** (IS-A relationship).
* But the subclass may have extra methods that the superclass doesn’t know about.

---

## **3️⃣ Which methods can be called?**
...
ref.methodThree(); // ❌ Not allowed, not visible in SuperClass
```
...
* Since the actual object is `SubClass`, **SubClass’s methodTwo()** is executed:
...
## **5️⃣ Why dynamic method dispatch? (Philosophy)**
...
| Allowed?               | Superclass reference can hold subclass object ✅    |
| Not allowed            | Subclass reference cannot hold superclass object ❌ |
...
// obj.methodThree(); // ❌ Compile-time error
```

* `methodOne()` → superclass only → executed as defined in superclass.
* `methodTwo()` → overridden → executed as defined in subclass.
* `methodThree()` → compiler error → reference doesn’t know about it.
...
* ❌ Changing parameter types or adding parameters → becomes **method overloading**, not overriding.
...
class SubClass extends SuperClass {
    @Override
    public B getObject() { return new B(); } // ✅ Allowed (covariant)
}
```
...
* `private` methods → cannot be overridden (not visible outside the class)

---

## **5️⃣ Use of `@Override` Annotation**
...
* Overloading ≠ overriding. Overloading = same method name, **different parameters**.
...
class SubClass extends SuperClass {
    ...
    // Cannot override finalMethod → ❌ error
    // Cannot override staticMethod → ❌ error
}
```
...
public class Main {
    public static void main(String[] args) {
        ...
        Super s2 = new Sub();
        s2.display(); // Sub display → dynamic method dispatch
    }
}
```
...
## **3️⃣ Differences Between Overloading and Overriding**
...
class Sub extends Super {
    @Override
    void display() {       // Overriding abstract method
        System.out.println("Sub display");
    }
}
